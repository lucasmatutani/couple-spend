...
export default function CreditCardCategoryChart({ open, onClose, expenses }: Props) {
  const ccExpenses = expenses.filter(
    (e) => e.paymentMethod === 'credit_card' && !e.reimbursed && e.categoryName !== 'Reembolsos',
  )

  const totals = new Map<string, number>()
  for (const e of ccExpenses) {
    const effective = Math.round(e.amountCents / e.splitParts)
    totals.set(e.categoryName, (totals.get(e.categoryName) ?? 0) + effective)
  }
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => { if (!v) onClose() }}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>Cartão por categoria</DialogTitle>
        </DialogHeader>
